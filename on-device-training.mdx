...
There are 3 main configurations that are passed to a simplfiine trainer function. The SFTConfig from huggingface's trl library, the prompt config indicating the prompt template, response tempalte and so on, and the wandb config for logging. 
